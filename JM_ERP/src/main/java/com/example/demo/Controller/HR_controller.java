...
// 근로계약서 수정
	...
	public String updateCont(@PathVariable("id") int id, Model model) {
		...
		model.addAttribute("HR_contCreateForm", form);

		return "HR_contCreate"; // 등록 폼 그대로 사용
	}

	@PostMapping("/cont/update/{id}")
	public String updateCont(@PathVariable("id") int id, @Valid HR_contCreateForm contCreateForm, BindingResult bindingResult, Model model) {
		if (bindingResult.hasErrors()) {
			// 유효성 검사에 실패한 경우, 동일한 폼으로 돌아가 오류 메시지 표시
			return "HR_contCreate";
		}
		contService.updateCont(contCreateForm.getId(), contCreateForm);
		return "redirect:/HR/cont/list";
	}
